package newFeeDelegatedSmartContractDeployWithRatioTC
...
const Name = "newFeeDelegatedSmartContractDeployWithRatioTC"
...
func Run() {
	...
	if err == nil {
		boomer.Events.Publish("request_success", "http", "transferNewFeeDelegatedSmartContractDeployWithRatioTx"+" to "+endPoint, elapsed, int64(10))
		cliPool.Free(cli)
	} else {
		boomer.Events.Publish("request_failure", "http", "transferNewFeeDelegatedSmartContractDeployWithRatioTx"+" to "+endPoint, elapsed, err.Error())
	}
}
...
func RunSingle() (txHash common.Hash, err error) {
	...
	txHash, _, err = from.TransferNewFeeDelegatedSmartContractDeployWithRatioTx(cli, to, value)
	if err != nil {
		return common.Hash{}, err
	}
