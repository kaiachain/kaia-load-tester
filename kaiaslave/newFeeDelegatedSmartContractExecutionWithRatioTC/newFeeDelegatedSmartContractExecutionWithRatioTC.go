package newFeeDelegatedSmartContractExecutionWithRatioTC
...
const Name = "newFeeDelegatedSmartContractExecutionWithRatioTC"
...
func Run() {
	...
	if err == nil {
		boomer.Events.Publish("request_success", "http", "transferNewFeeDelegatedSmartContractExecutionWithRatioTx"+" to "+endPoint, elapsed, int64(10))
		cliPool.Free(cli)
	} else {
		boomer.Events.Publish("request_failure", "http", "transferNewFeeDelegatedSmartContractExecutionWithRatioTx"+" to "+endPoint, elapsed, err.Error())
	}
}
...
func RunSingle() (txHash common.Hash, err error) {
	...
	txHash, _, err = from.TransferNewFeeDelegatedSmartContractExecutionWithRatioTx(cli, to, value)
	if err != nil {
		return common.Hash{}, err
	}
