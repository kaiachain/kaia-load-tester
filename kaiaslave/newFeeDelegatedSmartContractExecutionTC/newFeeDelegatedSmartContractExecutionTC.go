package newFeeDelegatedSmartContractExecutionTC
...
const Name = "newFeeDelegatedSmartContractExecutionTC"
...
func Run() {
	...
	start := boomer.Now()
	_, _, err := from.TransferNewFeeDelegatedSmartContractExecutionTx(cli, SmartContractAccount, value)
	elapsed := boomer.Now() - start

	if err == nil {
		boomer.Events.Publish("request_success", "http", "transferNewFeeDelegatedSmartContractExecutionTx"+" to "+endPoint, elapsed, int64(10))
		cliPool.Free(cli)
	} else {
		boomer.Events.Publish("request_failure", "http", "transferNewFeeDelegatedSmartContractExecutionTx"+" to "+endPoint, elapsed, err.Error())
	}
}
...
func RunSingle() (txHash common.Hash, err error) {
	...
	txHash, _, err = from.TransferNewFeeDelegatedSmartContractExecutionTx(cli, to, value)
	if err != nil {
		return common.Hash{}, err
	}
