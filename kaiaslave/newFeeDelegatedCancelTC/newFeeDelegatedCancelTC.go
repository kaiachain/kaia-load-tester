package newFeeDelegatedCancelTC
...
const Name = "newFeeDelegatedCancelTC"
...
func Run() {
	...
	if err == nil {
		boomer.Events.Publish("request_success", "http", "transferNewFeeDelegatedCancelTx"+" to "+endPoint, elapsed, int64(10))
		cliPool.Free(cli)
	} else {
		boomer.Events.Publish("request_failure", "http", "transferNewFeeDelegatedCancelTx"+" to "+endPoint, elapsed, err.Error())
	}
}
...
func RunSingle() (txHash common.Hash, err error) {
	...
	txHash, _, err = from.TransferNewFeeDelegatedCancelTx(cli, to, value)
	if err != nil {
		return common.Hash{}, err
	}
