...
func Run() {
	...
	if err == nil {
		boomer.Events.Publish("request_success", "http", "unsignedtransfer"+" to "+endPoint, elapsed, int64(10))
		cliPool.Free(cli)
	} else {
		boomer.Events.Publish("request_failure", "http", "unsignedtransfer"+" to "+endPoint, elapsed, err.Error())
	}
}
...
func RunSingle() (txHash common.Hash, err error) {
	...
	txHash, err = from.TransferUnsignedTx(cli, to, value)
	if err != nil {
		return common.Hash{}, err
	}
...
func CheckResult() (bool, error) {
	...
	fmt.Printf("[TC] transferUnsignedTc/CheckResult: %v, From: %v, To: %v\n", fromAccount.GetAddress().String(), toAccount.GetAddress().String())
