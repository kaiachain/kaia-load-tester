package newEthereumAccessListTC
...
const Name = "newEthereumAccessListTC"
...
func Run() {
	...
	if err != nil {
		boomer.Events.Publish("request_failure", "http", "transferNewEthereumAccessListTx"+" to "+endPoint, elapsed, err.Error())
	}
	...
	go func(transactionHash common.Hash) {
		...
		boomer.Events.Publish("request_success", "http", "transferNewEthereumAccessListTx"+" to "+endPoint, elapsed, int64(10))
	}(txHash)
}
...
func CheckResult(txHash common.Hash, reqType int) (bool, error) {
	...
	if status == hexutil.Uint(types.ReceiptStatusFailed).String() {
