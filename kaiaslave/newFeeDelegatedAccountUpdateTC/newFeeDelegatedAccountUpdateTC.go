package newFeeDelegatedAccountUpdateTC
...
const Name = "newFeeDelegatedAccountUpdateTC"
...
func Run() {
	...
	if err == nil {
		boomer.Events.Publish("request_success", "http", "transferNewFeeDelegatedAccountUpdateTx"+" to "+endPoint, elapsed, int64(10))
		cliPool.Free(cli)
	} else {
		boomer.Events.Publish("request_failure", "http", "transferNewFeeDelegatedAccountUpdateTx"+" to "+endPoint, elapsed, err.Error())
	}
}
...
func RunSingle() (txHash common.Hash, err error) {
	...
	txHash, _, err = from.TransferNewFeeDelegatedAccountUpdateTx(cli, to, value)
	if err != nil {
		return common.Hash{}, err
	}
