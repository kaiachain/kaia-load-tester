package newValueTransferWithCancelTC
...
const Name = "newValueTransferWithCancelTC"
...
func Run() {
	...
	if err == nil {
		boomer.Events.Publish("request_success", "http", "transferNewValueTransferWithCancelTx"+" to "+endPoint, elapsed, int64(10))
		cliPool.Free(cli)
	} else {
		boomer.Events.Publish("request_failure", "http", "transferNewValueTransferWithCancelTx"+" to "+endPoint, elapsed, err.Error())
	}
}
...
func RunSingle() (txHash common.Hash, err error) {
	...
	txHash, _, err = from.TransferNewValueTransferTx(cli, to, value)
	if err != nil {
		return common.Hash{}, err
	}
