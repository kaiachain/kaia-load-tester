package newFeeDelegatedCancelWithRatioTC
...
const Name = "newFeeDelegatedCancelWithRatioTC"
...
func Run() {
	...
	if err == nil {
		boomer.Events.Publish("request_success", "http", "transferNewFeeDelegatedCancelWithRatioTx"+" to "+endPoint, elapsed, int64(10))
		cliPool.Free(cli)
	} else {
		boomer.Events.Publish("request_failure", "http", "transferNewFeeDelegatedCancelWithRatioTx"+" to "+endPoint, elapsed, err.Error())
	}
}
...
func RunSingle() (txHash common.Hash, err error) {
	...
	txHash, _, err = from.TransferNewFeeDelegatedCancelWithRatioTx(cli, to, value)
	if err != nil {
		return common.Hash{}, err
	}
