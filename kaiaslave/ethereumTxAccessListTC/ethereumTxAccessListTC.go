package ethereumTxAccessListTC
...
const Name = "ethereumTxAccessListTC"
...
func Run() {
	...
	txHash, _, err := from.TransferNewEthAccessListTxWithEth(cli, endPoint, to, value, input, executablePath)

	elapsed := boomer.Now() - start

	if err != nil {
		boomer.Events.Publish("request_failure", "http", "TransferNewEthAccessListTx"+" to "+endPoint, elapsed, err.Error())
	}
	...
	go func(transactionHash common.Hash) {
		...
		boomer.Events.Publish("request_success", "http", "TransferNewEthAccessListTx"+" to "+endPoint, elapsed, int64(10))
	}(txHash)
}
...
func CheckResult(txHash common.Hash, reqType int) (bool, error) {
	...
	if typeString != types.TxTypeEthereumAccessList.String() {
	...
	if types.TxType(typeInt) != types.TxTypeEthereumAccessList {
