...
func (cfg *Config) setConfigsFromNode() {
	...
	// Create Cli pool
	if cfg.gCli, err = client.Dial(cfg.gEndpoint); err != nil {
		log.Fatalf("Failed to connect RPC: %v", err)
	}
	if cfg.ethCli, err = client.DialEth(cfg.gEndpoint); err != nil {
		log.Fatalf("Failed to connect RPC: %v", err)
	}
	...
	for {
		...
		chainID, err := cfg.ethCli.ChainID(ctx)
...
func (cfg *Config) GetGCli() *client.KaiaClient          { return cfg.gCli }
